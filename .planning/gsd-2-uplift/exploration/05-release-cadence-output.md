...
- scripts/update-changelog.mjs:~~78-136~~ → `:33-56` (post-audit corrected; see (vi)).
- scripts/generate-changelog.mjs:~~137-260~~ → `:1-152` (post-audit corrected; see (vi)).
...
**Finding 2.4 — release tooling recognizes semver-major triggers, but observed tags stay in major version 2 in the visible window (confidence: high).** The release workflow template says to propose a `major` bump if a commit has a `BREAKING CHANGE:` footer or `!` suffix (`src/resources/extensions/gsd/workflow-templates/release.md:36-40`). The changelog generator also sets `hasBreaking` when a subject includes `BREAKING CHANGE` or `!:` and maps that to `major` (`scripts/generate-changelog.mjs:`~~194-204~~`:58, 66-67`, `scripts/generate-changelog.mjs:`~~230-260~~`:96, 118-125` — post-audit corrected; see (vi)). The visible tags are all `v2.x.y`, from `v2.36.0` through `v2.78.1`, per the tag output above.
...
13. **Changelog generation and version bump scripts.** `scripts/generate-changelog.mjs` reads conventional commits since the last stable tag and computes bump/changelog/release-notes output (`scripts/generate-changelog.mjs:`~~137-260~~`:1-152` — post-audit corrected; see (vi)). `scripts/bump-version.mjs` updates root/workspace/platform/package-lock versions (`scripts/bump-version.mjs:1-77`). `scripts/update-changelog.mjs` inserts the entry under `[Unreleased]` and updates comparison links (`scripts/update-changelog.mjs:`~~78-136~~`:33-56` — post-audit corrected; see (vi)).
...
## (vi) Post-audit corrigenda — script-file citation line numbers (2026-04-28)

*Added by Claude (Opus 4.7) post-W2-audit per disposition recorded in `.planning/deliberations/2026-04-28-w2-audit-dispositions-and-synthesis-readiness.md` D2. Per `.planning/gsd-2-uplift/exploration/05-release-cadence-audit.md §1` spot-check 5, three script-file citations had incorrect line numbers (overshooting actual file length); content described was verified correct in all three cases. Strikethrough-with-correction is applied in-place at each occurrence above (sections (i), Q2 finding 2.4, Q2 finding 2.7, Q3 finding 3.6, item 13); this section documents the convention and the source-of-truth corrections.*

**Affected files and corrected ranges:**

- **`scripts/update-changelog.mjs`** (file is 59 lines, not the cited 78-136). Original cited range: `:78-136`. Corrected: `:33-56` (insert after `[Unreleased]`; update comparison links).
- **`scripts/generate-changelog.mjs`** (file is 152 lines, not the cited 137-260). Three citations corrected:
  - Original cited range `:137-260` (general "parses commits / outputs bump-changelog-release-notes" reference) → `:1-152` (full file is the relevant range).
  - Original cited range `:194-204` (`hasBreaking` detection) → `:58, 66-67` (`let hasBreaking = false` at line 58; `if (subject.includes("BREAKING CHANGE") || subject.includes("!:"))` ... `hasBreaking = true` at lines 66-67).
  - Original cited range `:230-260` (major-bump mapping) → `:96, 118-125` (`const bumpType = hasBreaking ? "major" : ...` at line 96; semver bump arithmetic at lines 118-125).

**Why strikethrough-and-correct rather than addendum-only:** the citation-chase use case (a future reader following a citation from this slice file) is broken by addendum-only — the dead line number stays in the slice body. Strikethrough preserves the original error (audit-trace per the project's traces-over-erasure discipline; cf. memory `feedback_methodology_and_philosophy.md`); inline correction makes the citation chase work. Auditor verified content described is accurate in all three cases; only the line numbers are wrong.
