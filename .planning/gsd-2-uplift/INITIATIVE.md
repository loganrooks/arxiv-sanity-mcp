...
Per-slice prompts, audit specs, synthesis spec, pilot-gate criteria, setup checklist (codex-CLI pitfall reminders; output paths), failure-mode handling, and mid-stream pivot protocol are codified in the **orchestration package** at `.planning/gsd-2-uplift/orchestration/` (per decision B6). The package contains: `OVERVIEW.md` (procedural shell + dispatch commands + dispositions log); `preamble.md` (common preamble prepended to each slice dispatch); 5 per-slice specs (`slice-01-mental-model.md` through `slice-05-release-cadence.md`); `audit-spec.md` (W2 audit template); `synthesis-spec.md` (W3 synthesis spec). Each slice dispatch concatenates `preamble.md` + the slice spec; the agent receives the full prompt as one document. The package is living — updates per wave; INITIATIVE.md (this artifact) stays focused on staging.
